...
Frontier Command is a real-time world-conquest game on a true political world map. Every country uses the same deterministic national AI for finance, development, recruitment, recovery and military operations. In a human-controlled country that autopilot is called **APEX**; selection grants no hidden planning or AI-efficiency advantage, while the human commander chooses war and peace. It does amplify that country's one existing trait, with a smooth 1.08× modifier for the strongest opening military through 1.80× for the weakest. Solo play assigns one country. Direct Connect multiplayer assigns a different country to each of 2–8 people in the same world, with the same economy, research, AI and combat rules. AI skill scales only modestly with the country's bounded national-IQ gameplay score, so a higher score means slightly better execution without creating a separate ruleset.
...
Every empire has exactly one immutable country trait: the trait of its original 2026 nation. All visible trait names, effects, conditions and identity text are English. Each trait is audited against a real opening weakness, and weaker opening militaries receive materially larger tools; Greenland, the weakest start, receives +1,200% Army capacity, +150% recruitment throughput and −80% upkeep. Its amplified upkeep can never fall below 10%, so even this extreme catch-up trait does not create a free army. A human seat amplifies the signed bonuses of that same trait using the 1.08×–1.80× opening-military-rank curve. Conquest, integration, revolution and federation never copy, donate or stack another country's trait.
...
Wars are persistent operations resolved on a deterministic combat cadence. Every viable, source-unique army route becomes a real simultaneous front; the complete deployed source and target armies on those fronts contribute to pressure, take losses, appear on the map and add operations cost. ATK, DEF, supply, condition, terrain, supporting armies and the defender's 25% position advantage all matter. Naval fronts now extend to long-haul global lanes. A short route starts at 1.35× land operating cost and 0.92× supply; distance raises cost smoothly toward 2.15× and lowers supply toward 0.62× by 9,000 km. Naval access has no separate assault or casualty penalty. Effective damage uses a `0.008` (0.8%) multiplier against the opposing pressure/shield ratio, exactly half of the former `0.016` baseline. There is no per-pulse casualty or damage ceiling: requested losses are applied directly, with only the remaining local manpower as the natural upper bound. A compact War Strain meter above the live war cards rates push sustainability from current wars, fronts, fatigue, active readiness and reserves, then states the existing growth, research and operation-cost consequences. Front weight now affects the opening load as well as the logarithmic extra-front term: one fresh land front remains 14 strain, while one fresh naval front is 10 because each naval route counts as 0.35 of a land front. Fatigue earned directly from a naval battle pulse is also multiplied by 0.65; conquest, treaty and post-war fatigue remain unchanged. Very high strain creates a visible strategic weakness: from score 65–85 AI countries increasingly prioritize and commit against a human empire, with bounded two- and three-attacker opportunities at scores 75 and 85. Those AI pressure wars remain independent, so every attacker and the world still respect their ordinary concurrent-war caps.
...
- Choose a country from the single pure military-ranking picker; its score is current Combat Power only. The same order is used by the picker, top bar and ranking display. The Army column combines deployed manpower and capacity as `x / x`. Selection opens the live map immediately, without an explainer or activation briefing.
...
- Scroll to zoom as deep as 24×, drag to pan and press `Esc` to close the current panel. The top ten powers keep strategic labels while close zoom reveals compact countries and integration progress. Land warfare follows a warm, solid ground-contact line with directional armored chevrons and shock impacts; naval warfare uses cool curved dashed sea lanes, fleet/wake markers and splash impacts. These screen-pixel-scaled effects remain legible at every zoom and reduce motion when the browser requests it.
