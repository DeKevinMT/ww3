...
export function processRogueAttentionV2(
  ...
  if (attention.stage === 'dormant') {
    ...
    addWorldEventV2(
      ...
      'polar',
      'critical',
      `ROGUE ATTENTION · OBSERVING: EONSCAR detects a response to ${Math.round(attention.liberatedWorldShare * 100)}% world liberation. Estimated buildup: ${estimatedBuildup} weeks.`,
      undefined,
      state.humanPlayerId,
      { polarRegion: 'antarctica' },
    );
