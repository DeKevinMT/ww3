...
describe('V2 battle collateral population damage', () => {
  it('applies local civilian losses once and exposes them to live economy and finance selectors', () => {
    ...
    expect(worldPopulationAfter).toBeCloseTo(
      worldPopulationBefore - event.attackerPopulationLoss - event.defenderPopulationLoss,
      5,
    );
  ...
  it('keeps deaths high on both sides while the defender remains only moderately worse hit', () => {
    const { state, war, operation } = battleFixture(7_701_1);
    ...
    const event = resolveBattlePulseV2(state, WORLD_CONTENT_V2, war, operation)!;
    const battleIntensity = event.attackerLosses + event.defenderLosses;
    const sourceExposure = civilianPopulationExposureV2(sourcePopulation);
    const targetExposure = civilianPopulationExposureV2(targetPopulation);
    const sourceTerrain = WORLD_CONTENT_V2.territories[belgiumTerritory].terrain;
    const targetTerrain = WORLD_CONTENT_V2.territories[netherlandsTerritory].terrain;
    const sourceRisk = sourceTerrain === 'urban' ? 1.20 : sourceTerrain === 'jungle' ? 1.08 : 1;
    const targetRisk = targetTerrain === 'urban' ? 1.35 : targetTerrain === 'jungle' ? 1.12 : 1;
    const retiredDefenderLoss = Math.min(
      targetPopulation * 0.002 * targetExposure,
      battleIntensity * 0.45 * targetExposure * targetRisk,
    );
    const retiredAttackerLoss = Math.min(
      sourcePopulation * 0.0004 * sourceExposure,
      retiredDefenderLoss * 0.20,
      battleIntensity * 0.08 * sourceExposure * sourceRisk,
    );
    const currentTotal = event.attackerPopulationLoss + event.defenderPopulationLoss;
    const retiredTotal = retiredAttackerLoss + retiredDefenderLoss;
    const defenderToAttacker = event.defenderPopulationLoss / event.attackerPopulationLoss;

    expect(DEFENDER_CIVILIAN_LOSS_INTENSITY).toBe(0.90);
    expect(ATTACKER_CIVILIAN_LOSS_INTENSITY).toBe(0.55);
    expect(defenderToAttacker).toBeGreaterThanOrEqual(1.5);
