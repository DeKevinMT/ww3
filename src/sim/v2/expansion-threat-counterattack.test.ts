...
describe('recent expansion and neighbour counterattacks', () => {
  it('keeps the neighbour-risk curve monotone across guarded, high and critical thresholds', () => {
    ...
    const risks = scores.map((score) => counterattackRiskForExpansionThreatV2(score, true));
  ...
  it('lets suspicion amplify only an existing human-neighbour opening', () => {
    const expansionOnly = aiCounterattackPressureWithSuspicionV2(0.50, 0, true);
    const combined = aiCounterattackPressureWithSuspicionV2(0.50, 100, true);
    expect(expansionOnly).toBe(0.50);
  ...
  it('keeps conquest history separate while battle fatigue recovers in peacetime', () => {
